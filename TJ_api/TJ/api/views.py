...
def goal_list(request):
    ...
    return render(request, 'api/goal_list.html', context)
...
def edit_goal(request, pk):
    goal = get_object_or_404(Goal, pk=pk, user=request.user)  # Ensure the goal belongs to the current user
    if request.method == 'POST':
...
def leave_detail(request, pk):
    leave = get_object_or_404(Leave, pk=pk, user=request.user)  # Ensure the leave belongs to the current user
...
def delete_leave(request, pk):
    ...
    if request.method == 'POST':
        ...
        return redirect('leave_list')
    return render(request, 'api/confirm_delete.html', {'object': leave, 'type': 'leave'})
